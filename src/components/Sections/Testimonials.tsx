...
import {memo, UIEventHandler, useCallback, useEffect, useMemo, useRef, useState} from 'react';
    ...
    <Section noPadding sectionId={SectionId.Testimonials}>
      <div
        className={classNames(
          ...
          {'bg-retro-surface': !imageSrc},
        )}
        style={imageSrc ? {backgroundImage: `url(${resolveSrc}`} : undefined}>
        <div className="z-10 w-full max-w-screen-md px-4 lg:px-0">
          <div className="flex flex-col items-center gap-y-6 border border-retro-border bg-retro-surface/60 p-6 shadow-lg">
            <div
            ...
            <div className="flex gap-x-4">
              {[...Array(testimonials.length)].map((_, index) => {
                ...
                return (
                  <button
                    className={classNames(
                      'h-3 w-3 bg-retro-muted transition-all duration-500 sm:h-4 sm:w-4',
...
const Testimonial = memo(
  ({testimonial: {text, name, image}, isActive}: {testimonial: Testimonial; isActive: boolean}) => (
    <div
      className={classNames(
      ...
      {image ? (
        <div className="relative h-14 w-14 shrink-0 sm:h-16 sm:w-16">
          <QuoteIcon className="absolute -left-2 -top-2 h-4 w-4 stroke-retro-border text-retro-green" />
          <img className="h-full w-full rounded-full" src={image} />
        </div>
      ) : (
        <QuoteIcon className="h-5 w-5 shrink-0 text-retro-green sm:h-8 sm:w-8" />
      )}
      <div className="flex flex-col gap-y-4">
        <p className="prose prose-sm font-medium italic text-retro-text sm:prose-base">{text}</p>
        <p className="text-xs italic text-retro-text sm:text-sm md:text-base lg:text-lg">-- {name}</p>
      </div>
    </div>
  ),
);
