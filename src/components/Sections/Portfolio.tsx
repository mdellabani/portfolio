...
const Portfolio = memo(() => {
  return (
    <Section className="bg-retro-surface" sectionId={SectionId.Portfolio}>
      <div className="flex flex-col gap-y-8">
        <h2 className="self-center text-xl font-bold text-retro-text">Check out some of my work</h2>
              ...
              <div key={`${title}-${index}`}>
                <div
                  className={classNames(
                    'relative aspect-video w-full overflow-hidden rounded-lg shadow-lg shadow-black/30 lg:shadow-xl',
                  )}>
...
const ItemOverlay = memo(({item: {url, title, description}}: {item: PortfolioItem}) => {
  ...
  return (
    <a
      className={classNames(
        'absolute inset-0 h-full w-full bg-retro-base transition-all duration-300',
